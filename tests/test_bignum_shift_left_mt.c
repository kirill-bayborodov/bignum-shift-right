/**
 * @file    test_bignum_shift_left_mt.c
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    03.10.2025
 *
 * @brief   Тест на потокобезопасность для модуля bignum_shift_left.
 *
 * @details
...
int main() {
    printf("--- Starting MT test for bignum_shift_left 1.0.0 ---\n");
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        ...
        //bignum_shift_left(&thread_data[i].expected, NUM_ITERATIONS);

        // Рассчитываем ожидаемый результат так же, как это делает поток
        thread_data[i].expected = thread_data[i].num;
        for (int j = 0; j < NUM_ITERATIONS; ++j) {
            bignum_shift_left(&thread_data[i].expected, 1);
        }
    ...
    printf("--- MT test for bignum_shift_left 1.0.0 passed ---\n");
    return 0;
}
