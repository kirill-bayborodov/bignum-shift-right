/**
 * @file    test_bignum_shift_left.c
 * @author  git@bayborodov.com
 * @version 1.0.0
 * @date    03.10.2025
 *
 * @brief   Детерминированные тесты для модуля bignum_shift_left.
 *
 * @details
...
void test_shift_zero_amount() { printf("Running test: %s\n", __func__); bignum_t n = {.words = {1,1}, .len = 2}, e = n; assert(bignum_shift_left(&n, 0) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e)); printf("...PASSED\n"); }
void test_shift_zero_number() { 
    printf("Running test: %s\n", __func__);
    bignum_t n = {.words = {0}, .len = 1}, e = n;
    assert(bignum_shift_left(&n, 100) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e));
    printf("...PASSED\n"); 
   }
...
void test_simple_bit_shift() { 
    ...
    assert(bignum_shift_left(&n, 2) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e)); 
    printf("...PASSED\n"); 
}
...
void test_bit_shift_with_carry() { 
    ...
    assert(bignum_shift_left(&n, 1) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e)); 
    printf("...PASSED\n"); 
}
void test_exact_word_shift() { 
    printf("Running test: %s\n", __func__); 
    bignum_t n = {.words = {1,2}, .len = 2}, e = {.words = {0,1,2}, .len = 3}; 
    assert(bignum_shift_left(&n, 64) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e)); 
    printf("...PASSED\n"); 
}
void test_mixed_shift() { 
    printf("Running test: %s\n", __func__); 
    bignum_t n = {.words = {1}, .len = 1}, e = {.words = {0, 0x8000000000000000}, .len = 2};
    assert(bignum_shift_left(&n, 127) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e));
    printf("...PASSED\n"); 
  }
//void test_overflow_shift() { printf("Running test: %s\n", __func__); bignum_t n = {.words = {0,0,0,0,0,0x8000000000000000}, .len = 6}; assert(bignum_shift_left(&n, 1) == BIGNUM_SHIFT_ERROR_OVERFLOW); printf("...PASSED\n"); }
/**
 ...
 *   - Копируем n_orig в n.
 *   - Вызываем bignum_shift_left(&n, 1).
 *   - Ожидаем код BIGNUM_SHIFT_ERROR_OVERFLOW.
...
static void test_overflow_shift(void) {
    ...
    bignum_shift_status_t rc = bignum_shift_left(&n, 1);
...
void test_shift_to_boundary() { 
    ...
    assert(bignum_shift_left(&n, 383) == BIGNUM_SHIFT_SUCCESS && bignum_are_equal(&n, &e)); 
    printf("...PASSED\n"); 
}
...
void test_empty_len_zero() {
    printf("Running test: %s\n", __func__);
    bignum_t num = {.len = 0};
    assert(bignum_shift_left(&num, 10) == BIGNUM_SHIFT_SUCCESS);
    assert(num.len == 0);
    printf("...PASSED\n");
}
...
void test_null_arg() {
    printf("Running test: %s\n", __func__);
    assert(bignum_shift_left(NULL, 10) == BIGNUM_SHIFT_ERROR_NULL_ARG);
    printf("...PASSED\n");
}
...
void test_shift_too_large() {
    ...
    assert(bignum_shift_left(&num, big_shift) == BIGNUM_SHIFT_ERROR_OVERFLOW);
...
void test_carry_across_multiple_words() {
    ...
    assert(bignum_shift_left(&num, 1) == BIGNUM_SHIFT_SUCCESS);
...
void test_shift_and_truncate() {
    ...
    assert(bignum_shift_left(&num, 128) == BIGNUM_SHIFT_SUCCESS);
    assert(bignum_are_equal(&num, &expected));
    printf("...PASSED\n");
}

int main() {
    printf("--- Starting tests for bignum_shift_left 1.0.0 ---\n");
    ...
    printf("--- All tests for bignum_shift_left 1.0.0 passed ---\n");
    return 0;
}
